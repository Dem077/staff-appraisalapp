...
class ViewAssigneeResults extends Page implements HasTable
{
    ...
    protected string $view = 'filament.staff.resources.forms-assigned-to-hod-resource.pages.view-assignee-results';
        ...
        return $table
            ...
            ->filters([
                SelectFilter::make('hod_form_assignee_id')
                    ->label('Assignee Name')
                    ->options(
                        HodFormassignee::where('forms_assigned_to_hod_id', $this->record->id)
                            ->with('staff')
                            ->get()
                            ->pluck('staff.name', 'id')
                            ->toArray()
