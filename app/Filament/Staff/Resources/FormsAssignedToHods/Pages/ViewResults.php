...
class ViewResults extends Page implements HasTable
{
    ...
    protected string $view = 'filament.staff.resources.forms-assigned-to-hod-resource.pages.view-results';
        ...
        return $schema
            ...
            ->schema([
                ...
                Section::make('Comments')
                    ...
                    ->headerActions([
                        Action::make('add_supervisor_comment')
            ...
            ->columns([
                ...
                TextColumn::make('assignee_subordinate.score')
                    ...
                    ->getStateUsing(function ($record) {
                        return HodFormAssigneeEntry::whereHas('hodFormAssignee', function ($q) use ($record) {
                    ...
                    ->summarize(Summarizer::make()
                        ...
                        ->using(fn (\Illuminate\Database\Query\Builder $query) => (
                                HodFormAssigneeEntry::whereHas('hodFormAssignee', function ($q) {
                ...
                TextColumn::make('total_score')
                    ...
                    ->summarize(Summarizer::make()
                        ...
                        ->using(fn (\Illuminate\Database\Query\Builder $query) => (
                                HodFormAssigneeEntry::whereHas('hodFormAssignee', function ($q) {
                                    $q->where('forms_assigned_to_hod_id', $this->record->id)
                                        ->orwhere('assignee_type', HODFormassigneeType::Manager->value)
                                        ->orwhere('assignee_type', HODFormassigneeType::CoWorker->value)
                                        ->orwhere('assignee_type', HODFormassigneeType::Subordinate->value);
                                })
                                    ->where('hidden', false)
                                    ->sum('score')
                                /
    ...
    {
        $managerComment = HodFormAssigneeEntry::whereHas('hodFormAssignee', function ($q) use ($record) {
            $q->where('forms_assigned_to_hod_id', $record->forms_assigned_to_hod_id)
                ->where('assignee_type', HODFormassigneeType::Manager->value);
        })->where('hidden', false)
            ->where('question_id', $record->question_id)
        ...
        $subComment = HodFormAssigneeEntry::whereHas('hodFormAssignee', function ($q) use ($record) {
            $q->where('forms_assigned_to_hod_id', $record->forms_assigned_to_hod_id)
                ->where('assignee_type', HODFormassigneeType::Subordinate->value);
        })->where('hidden', false)
            ->where('question_id', $record->question_id)
        ...
        $coworkerComment = HodFormAssigneeEntry::whereHas('hodFormAssignee', function ($q) use ($record) {
            $q->where('forms_assigned_to_hod_id', $record->forms_assigned_to_hod_id)
                ->where('assignee_type', HODFormassigneeType::CoWorker->value);
        })->where('hidden', false)
            ->where('question_id', $record->question_id)
    ...
    {
        return HodFormAssigneeEntry::whereHas('hodFormAssignee', function ($q) use ($record ,  $assigneetype) {
            $q->where('forms_assigned_to_hod_id', $record->forms_assigned_to_hod_id)
